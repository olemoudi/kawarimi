// Package simenv provides in-process simulations of every external actor kawarimi
// talks to — SMTP, Telegram, the GitHub API, the cloud DMS git repo, and the
// GitHub Actions workflow runner. It contains no testing imports: constructors
// return errors, so the same actors serve both the test harness (internal/testenv
// wraps them with testing.TB conveniences) and the live sandbox behind
// `kawarimi demo`.
package simenv
...
// StartMail starts a mail server on a random loopback port.
func StartMail() (*MailServer, error) {
	ln, err := net.Listen("tcp", "127.0.0.1:0")
	if err != nil {
		return nil, fmt.Errorf("start mail server: %w", err)
	}
...
func (m *MailServer) handle(conn net.Conn) {
	...
	line("220 mock.simenv ESMTP ready")
	var from string
	var rcpts []string
	...
	for {
		...
		case strings.HasPrefix(up, "EHLO"):
			fmt.Fprint(w, "250-mock.simenv\r\n250-AUTH PLAIN LOGIN\r\n250 OK\r\n")
			w.Flush()
		case strings.HasPrefix(up, "HELO"):
			line("250 mock.simenv")
