...
// so callers can assert on (or reuse) their plaintext. Route the production client
// here by setting KAWARIMI_GITHUB_API to URL().
//
// RepoSSHURL is returned as the created repo's ssh_url; point it at a local bare
// repo so that SeedSwitch pushes the heartbeat locally instead of over the network.
...
// StartGitHub starts the mock. It does not touch the environment.
func StartGitHub() (*GitHubServer, error) {
	pub, priv, err := box.GenerateKey(rand.Reader)
	if err != nil {
		return nil, fmt.Errorf("github mock keypair: %w", err)
	}
...
// URL returns the base URL to set as KAWARIMI_GITHUB_API.
func (g *GitHubServer) URL() string { return g.srv.URL }

// Close stops the mock.
func (g *GitHubServer) Close() { g.srv.Close() }
